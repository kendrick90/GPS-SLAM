									   ...
									   Matrix4f invM, Vector4f invProjParams, float oneOverVoxelSize, float mu, const CONSTPTR(Vector2f) & viewFrustum_minmax)
{
	...
	// if (x == 234 && y == 346)
	// 	printf("step_scale: %f, totalLength: %f, total_length: %f\n", stepScale, totalLength, totalLengthMax);
	while (totalLength < totalLengthMax)
	{
		...
		// stepLength = 1;
		// if (x == 234 && y == 346)
		// 	printf("sdfValue: %f, stepLength: %f, pt: (%f, %f, %f)\n", sdfValue, stepLength, pt_result.x, pt_result.y, pt_result.z);
		pt_result += stepLength * rayDirection;
		totalLength += stepLength;
	}
	// 如果查询得到的sdf值小于0，说明这个光线是穿过了有效的表面
	if (sdfValue <= 0.0f)
	{
		...
		pt_found = false;
	// if (!pt_found)
	// 	printf("debug empty depth, x: %d, y: %d\n", x, y);
