...
inline void ITMMultiMeshingEngine_CPU<TVoxel, ITMVoxelBlockHash>::MeshScene(ITMMesh *mesh, const MultiSceneManager &sceneManager)
{
	...
	if (numLocalMaps > MAX_NUM_LOCALMAPS)
		numLocalMaps = MAX_NUM_LOCALMAPS;
			...
			for (int z = 0; z < SDF_BLOCK_SIZE; z++)
				for (int y = 0; y < SDF_BLOCK_SIZE; y++)
					for (int x = 0; x < SDF_BLOCK_SIZE; x++)
					{
						...
						// 	uninterpolate(localVBAs, hashTable, globalPos + Vector3i(threadIdx.x, threadIdx.y, threadIdx.z));
