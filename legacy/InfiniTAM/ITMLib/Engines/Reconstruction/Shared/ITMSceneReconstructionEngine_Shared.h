															  ...
															  float oneOverVoxelSize, const CONSTPTR(ITMHashEntry) * hashTable, float viewFrustum_min, float viewFrustum_max)
{
	...
	// if (x == 234 && y == 346)
	// 	printf("depth_measure: %f, mu: %f, viewFrustum_min: %f, viewFrustum_max: %f, oneOverVoxelSize: %f\n", depth_measure, mu, viewFrustum_min, viewFrustum_max, oneOverVoxelSize);
	...
	// if (x == 234 && y == 346)
	// {
	// 	printf("pt_camera_f: (%f, %f, %f), norm: %f\n", pt_camera_f.x, pt_camera_f.y, pt_camera_f.z, norm);
	// 	printf("point: (%f, %f, %f), point_e: (%f, %f, %f)\n", point.x, point.y, point.z, point_e.x, point_e.y, point_e.z);
	// 	printf("norm: %f, noSteps: %d\n", norm, noSteps);
	// }
	...
	for (int i = 0; i < noSteps; i++)
	{
		...
		// if (x == 234 && y == 346)
		// {
		// 	printf("isFound: %d, hashIdx: %d, hashEntry.ptr: %d\n", isFound, hashIdx, hashEntry.ptr);
		// }
		// if (hashIdx == 893845)
		// {
		// 	printf("x: %d, y: %d\n", x, y);
		...
		if (!isFound)
		{
			...
			if (!isFound) // still not found
			{
				...
				// if (x == 234 && y == 346)
				// {
				// 	printf("entriesAllocType: %u, entriesVisibleType: %u\n", entriesAllocType[hashIdx], entriesVisibleType[hashIdx]);
				// 	printf("blockCoords: (%hd, %hd, %hd)\n", blockPos.x, blockPos.y, blockPos.z);
				// 	printf("pointCoords: (%hd, %hd, %hd)\n", blockPos.x * 8, blockPos.y * 8, blockPos.z * 8);
				// }
