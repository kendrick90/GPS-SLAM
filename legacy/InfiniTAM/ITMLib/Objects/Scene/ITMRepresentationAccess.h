...
// 计算一个点在block坐标系下的坐标，并且返回这个点相对block中心内的线性偏移
...
{
	// 1.获取point对应的 block pos
	Vector3i blockPos;
	int linearIdx = pointToVoxelBlockPos(point, blockPos);
	// 如果这个block就是cache里缓存的block，直接返回这个block里对应的体素
	if IS_EQUAL3 (blockPos, cache.blockPos)
	{
		vmIndex = true;
		return voxelData[cache.blockPtr + linearIdx];
	}
	// 2. 获取block pos对应的哈希表索引并进行查询
	int hashIdx = hashIndex(blockPos);

	while (true)
	{
		ITMHashEntry hashEntry = voxelIndex[hashIdx];
		// 如果哈希表里对应的位置正好对应这个block的空间位置，更新cache，vmIndex和体素数据并返回
		if (IS_EQUAL3(hashEntry.pos, blockPos) && hashEntry.ptr >= 0)
		{
			cache.blockPos = blockPos;
			cache.blockPtr = hashEntry.ptr * SDF_BLOCK_SIZE3;
		...
		// 否则，检查这个哈希表空间是否还指向无序的、哈希冲突对应的哈希桶
		if (hashEntry.offset < 1)
			break;
		hashIdx = SDF_BUCKET_NUM + hashEntry.offset - 1;
	}
	// 没有找到对应的block，vmIndex返回false，并且返回一个空的体素
	vmIndex = false;
	return TVoxel();
}
																	...
																	const CONSTPTR(TIndex) * voxelIndex, const THREADPTR(Vector3f) & point, THREADPTR(TCache) & cache)
{
	...
	Vector3i pos;
	TO_INT_FLOOR3(pos, coeff, point);
	// if (pos.x == -122 && pos.y == 0 && pos.z == 363)
	// {
	// 	printf("point: (%f, %f, %f), coeff: (%f, %f, %f)\n", point.x, point.y, point.z, coeff.x, coeff.y, coeff.z);
	// }

	resn = readVoxel(voxelData, voxelIndex, pos + Vector3i(0, 0, 0), vmIndex, cache);
	if (resn.w_color >= w_color_thres)
	{
		ret += (1.0f - coeff.x) * (1.0f - coeff.y) * (1.0f - coeff.z) * resn.clr.toFloat();
		weight_sum += (1.0f - coeff.x) * (1.0f - coeff.y) * (1.0f - coeff.z);
		// if (pos.x == -122 && pos.y == 0 && pos.z == 363)
		// 	printf("read voxel color: (%f, %f, %f), vmIndex: %d, w_color: %u\n", resn.clr.toFloat().x, resn.clr.toFloat().y, resn.clr.toFloat().z, vmIndex, resn.w_color);
	}
	...
	{
		ret += (coeff.x) * (1.0f - coeff.y) * (1.0f - coeff.z) * resn.clr.toFloat();
		weight_sum += (coeff.x) * (1.0f - coeff.y) * (1.0f - coeff.z);
	}
	...
	{
		ret += (1.0f - coeff.x) * (coeff.y) * (1.0f - coeff.z) * resn.clr.toFloat();
		weight_sum += (1.0f - coeff.x) * (coeff.y) * (1.0f - coeff.z);
	}
	...
	{
		ret += (coeff.x) * (coeff.y) * (1.0f - coeff.z) * resn.clr.toFloat();
		weight_sum += (coeff.x) * (coeff.y) * (1.0f - coeff.z);
	}
	...
	{
		ret += (1.0f - coeff.x) * (1.0f - coeff.y) * coeff.z * resn.clr.toFloat();
		weight_sum += (1.0f - coeff.x) * (1.0f - coeff.y) * coeff.z;
	}
	...
	{
		ret += (coeff.x) * (1.0f - coeff.y) * coeff.z * resn.clr.toFloat();
		weight_sum += (coeff.x) * (1.0f - coeff.y) * coeff.z;
	}
	...
	{
		ret += (1.0f - coeff.x) * (coeff.y) * coeff.z * resn.clr.toFloat();
		weight_sum += (1.0f - coeff.x) * (coeff.y) * coeff.z;
	}
	...
	{
		ret += (coeff.x) * (coeff.y) * coeff.z * resn.clr.toFloat();
		weight_sum += (coeff.x) * (coeff.y) * coeff.z;
	}
	ret /= weight_sum;
	ret4.x = ret.x;
	ret4.y = ret.y;
	...
	ret4.w = 255.0f;
	// if (pos.x == -122 && pos.y == 0 && pos.z == 363)
	// 	printf("ret4: (%f, %f, %f, %f), weight_sum :%f\n", ret4.x, ret4.y, ret4.z, ret4.w, weight_sum);
...
struct SurfelColourManipulator<false>
{
	...
	_CPU_AND_GPU_CODE_ static Vector3u read(const TSurfel &surfel)
	{
	 ...
	 */
	template <typename TSurfel>
	_CPU_AND_GPU_CODE_ static void write(TSurfel &surfel, const Vector3u &colour)
	{
		// No-op
	}
};
