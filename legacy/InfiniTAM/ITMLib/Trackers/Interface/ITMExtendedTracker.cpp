...
void ITMExtendedTracker::TrackCamera(ITMTrackingState *trackingState, const ITMView *view)
{
	...
	// std::cout << "useDepth: " << useDepth << "\tuseColour: " << useColour << std::endl;
	for (int levelId = viewHierarchy_Depth->GetNoLevels() - 1; levelId >= 0; levelId--)
	{
		SetEvaluationParams(levelId);

		if (currentIterationType == TRACKER_ITERATION_NONE)
			continue;
		...
		for (int iterNo = 0; iterNo < noIterationsPerLevel[levelId]; iterNo++)
		{
			...
			if (useDepth)
			{
				...
				if (noValidPoints_depth > MIN_VALID_POINTS_DEPTH)
				{
					...
					for (int i = 0; i < 6 * 6; ++i)
						hessian_depth[i] /= noValidPoints_depth;
					for (int i = 0; i < 6; ++i)
						nabla_depth[i] /= noValidPoints_depth;
			...
			if (useColour)
			{
				...
				if (noValidPoints_RGB > MIN_VALID_POINTS_DEPTH)
				{
					// Normalize nabla and hessian
					for (int i = 0; i < 6 * 6; ++i)
						hessian_RGB[i] /= noValidPoints_RGB;
					for (int i = 0; i < 6; ++i)
						nabla_RGB[i] /= noValidPoints_RGB;
			...
			if (useDepth && useColour)
			{
				...
				if (noValidPoints_RGB > MIN_VALID_POINTS_RGB)
				{
					...
					f_new += f_RGB;
					for (int i = 0; i < 6; ++i)
						nabla_new[i] += colourWeight * nabla_RGB[i];
					for (int i = 0; i < 6 * 6; ++i)
						hessian_new[i] += colourWeight * colourWeight * hessian_RGB[i];
				}
			}
			else if (useDepth)
			{
			...
			else
			{
				...
				for (int i = 0; i < 6 * 6; ++i)
					hessian_good[i] = hessian_new[i];
				for (int i = 0; i < 6; ++i)
					nabla_good[i] = nabla_new[i];
				lambda /= 10.0f;
