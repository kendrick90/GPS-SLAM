...
namespace gsplat
{
    ...
    __global__ void rasterize_to_pixels_bwd_ges_kernel(
        const uint32_t C,
        const uint32_t N,
        ...
        int64_t *__restrict__ compute_count,
    ...
    )
    {
        ...
        const S ref_depth = ref_depth_map[pix_id];
        ...
        for (uint32_t b = 0; b < num_batches; ++b)
        {
            ...
            {
                bool valid = inside;
                if (rgbs_batch[t * COLOR_DIM + 3] > ref_depth + delta_depth)
                    valid = 0;
                S alpha;
                S opac;
                vec2<S> delta;
                ...
                // if (id_batch[t] == 803991 && i == 108)
                // {
                //     printf("pixel id: (%d, %d), depth: %f, %f, delta: %f, %f, conic: %f, %f, %f, sigma: %f, alpha: %f, valid: %d\n",
                //            i, j, rgbs_batch[t * COLOR_DIM + 3], ref_depth + delta_depth, delta.x, delta.y, conic.x, conic.y, conic.z, sigma, alpha, valid);
                // }
                ...
                if (valid)
                {
                    ...
                    v_alpha += v_render_a;
                    // if (id_batch[t] == 803991)
                    // {
                    //     printf("pixel id: (%d, %d), v_color: %f, %f, %f, v_alpha: %f, sigma: %f, alpha: %f\n",
                    //            i, j, v_rgb_local[0] * 1e6, v_rgb_local[1] * 1e6, v_rgb_local[2] * 1e6, v_alpha * 1e6, sigma, alpha);
                    // }
                    if (opac * vis <= 0.999f)
                    {
                        ...
                        // TODO: finish abs grad
                        if (v_means2d_abs != nullptr)
                        {
                            v_xy_abs_local = {abs(v_xy_local.x), abs(v_xy_local.y)};
                        }
                        v_opacity_local = vis * v_alpha;
                        // if (id_batch[t] == 414976)
                        // {
                        //     printf("pixel id: (%d, %d), delta: %f, %f, conic: %f, %f, %f, sigma: %f, alpha: %f\n",
                        //            i, j, delta.x, delta.y, conic.x, conic.y, conic.z, sigma, alpha);
                        // }
                    }
                }
        ...
        bool absgrad)
    {
        ...
        torch::Tensor compute_count = torch::zeros({1}, v_means2d.options()).to(torch::kInt64);
        torch::Tensor valid_count = torch::zeros({1}, v_means2d.options()).to(torch::kInt64);
        if (absgrad)
        {
        ...
        if (n_isects)
        {
            ...
            if (cudaFuncSetAttribute(
                    rasterize_to_pixels_bwd_ges_kernel<CDIM, float>,
            ...
            rasterize_to_pixels_bwd_ges_kernel<CDIM, float>
                <<<blocks, threads, shared_mem, stream>>>(
                    ...
                    image_width,
                    image_height,
                    compute_count.data_ptr<int64_t>(),
            ...
            // std::ofstream outfile("opac_grad.txt");
            // // 遍历 tensor
            // for (int64_t i = 0; i < v_opacities.size(0); ++i)
            // {
            //     float value = v_opacities[i].item<float>();
            //     if (value != 0)
            //     {
            //         outfile << "Index: " << i << ", Value: " << value << std::endl;
            //     }
            // }
        }
    ...
    std::tuple<
        ...
        torch::Tensor,
        torch::Tensor>
    rasterize_to_pixels_bwd_ges_tensor(
        // Gaussian parameters
        const torch::Tensor &means2d,                   // [C, N, 2] or [nnz, 2]
        const torch::Tensor &conics,                    // [C, N, 3] or [nnz, 3]
        const torch::Tensor &colors,                    // [C, N, 3] or [nnz, 3]
